              ...
              <div key={project.id} className="project-card">
                {project.photo && (
                  <div className="project-img-wrapper">
                    <img src={project.photo} alt={project.name} className="project-img" loading="lazy" decoding="async" />
                  </div>
                )}
                <div className="project-content">
                  <h3 className="project-title">{project.name}</h3>
                  <p className="project-desc project-desc-truncated">{project.description}</p>
                  <Link href={`/projects/${project.id}`} className="btn-learn-more" aria-label={`Learn more about ${project.name}`}>
